#include "ThermalExpansionHeatSourceFiniteStrainMieGruneisenConst.h"

registerMooseObject("pandaApp",ThermalExpansionHeatSourceFiniteStrainMieGruneisenConst);

template <>
InputParameters
validParams<ThermalExpansionHeatSourceFiniteStrainMieGruneisenConst>()
{
...
ThermalExpansionHeatSourceFiniteStrainMieGruneisenConst::ThermalExpansionHeatSourceFiniteStrainMieGruneisenConst(const InputParameters & parameters)
...
Real
ThermalExpansionHeatSourceFiniteStrainMieGruneisenConst::computeQpResidual()
{
  Real heat_source, Je;
  ...
  viscous_energy  = _C0 * trD * std::abs(trD) * _density[_qp] * _h_e * _h_e ;
  viscous_energy += _C1 * trD * _density[_qp] *  _h_e;
...
Real
ThermalExpansionHeatSourceFiniteStrainMieGruneisenConst::computeQpJacobian()
{
  Real Kb = _Bulk_Modulus_Ref;
...
Real
ThermalExpansionHeatSourceFiniteStrainMieGruneisenConst::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real val;
  Real Kb = _Bulk_Modulus_Ref;
  RankTwoTensor ce, ce_old, iden, ee, ee_old, ee_rate;
