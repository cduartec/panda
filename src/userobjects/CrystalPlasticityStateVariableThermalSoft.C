...
#include "CrystalPlasticityStateVariableThermalSoft.h"

#include <fstream>

registerMooseObject("pandaApp", CrystalPlasticityStateVariableThermalSoft);

template <>
InputParameters
validParams<CrystalPlasticityStateVariableThermalSoft>()
{
...
CrystalPlasticityStateVariableThermalSoft::CrystalPlasticityStateVariableThermalSoft(const InputParameters & parameters)
    ...
    _q(getParam<Real>("q")) 
{
  if (_scale_factor.size() != _num_mat_state_var_evol_rate_comps)
    mooseError("CrystalPlasticityStateVariableThermalSoft: Scale factor should be have the same size of "
...
void
CrystalPlasticityStateVariableThermalSoft::initSlipSysProps(std::vector<Real> & val,
                                                 const Point & q_point) const
{
  switch (_intvar_read_type)
  {
    ...
    case 2:
      ...
      break;
    default:
      mooseError("CrystalPlasticityStateVariableThermalSoft: Read option for initial value of internal "
                 "variables is not supported.");
  }

  for (unsigned int i = 0; i < _variable_size; ++i)
    if (val[i] <= 0.0)
      mooseError("CrystalPlasticityStateVariableThermalSoft: Value of state variables ", i, " non positive");
}

void
CrystalPlasticityStateVariableThermalSoft::readInitialValueFromFile(std::vector<Real> & val) const
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
    if (!(file >> val[i]))
      mooseError("Error CrystalPlasticityStateVariableThermalSoft: Premature end of state_variable file");

  file.close();
}

void
CrystalPlasticityStateVariableThermalSoft::readInitialValueFromInline(std::vector<Real> & val) const
{
  if (_groups.size() <= 0)
    mooseError("CrystalPlasticityStateVariableThermalSoft: Error in reading initial state variable values: "
               "Specify input in .i file or in state_variable file");
  else if (_groups.size() != (_group_values.size() + 1))
    mooseError(
        "CrystalPlasticityStateVariableThermalSoft: The size of the groups and group_values does not match.");
  ...
  for (unsigned int i = 0; i < _groups.size() - 1; ++i)
  {
    ...
    if (is > ie)
      mooseError("CrystalPlasticityStateVariableThermalSoft: Start index is = ",
                 is,
...
void
CrystalPlasticityStateVariableThermalSoft::provideInitialValueByUser(std::vector<Real> & /*val*/,
                                                          const Point & /*q_point*/) const
{
  mooseError("Error CrystalPlasticityStateVariableThermalSoft: User has to overwrite "
             "'provideInitialValueByUser' function"
             "in order to provide specific initial values based on quadrature point location.");
}

bool
CrystalPlasticityStateVariableThermalSoft::updateStateVariable(unsigned int qp,
                                                    Real dt,
                                                    std::vector<Real> & val,
                                                    std::vector<Real> & val_old) const
{ 
  const Real temp = _temp[qp];
  Real temp_fraction;
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    val[i] = 0.0;
    for (unsigned int j = 0; j < _num_mat_state_var_evol_rate_comps; j++){
      temp_fraction = ( 1.0 - std::pow( (temp - _temp_ref) / (_temp_melt - _temp_ref), _q));
      val[i] += (*_mat_prop_state_var_evol_rate_comps[j])[qp][i] * dt * _scale_factor[j]* temp_fraction;}
  }
