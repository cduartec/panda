...
registerMooseObject("pandaApp",FiniteStrainCrystalPlasticityDamageAmorMieGruneisen);
...
FiniteStrainCrystalPlasticityDamageAmorMieGruneisen::FiniteStrainCrystalPlasticityDamageAmorMieGruneisen(const InputParameters & parameters) :
...
void
FiniteStrainCrystalPlasticityDamageAmorMieGruneisen::preSolveStatevar()
{
...
void
FiniteStrainCrystalPlasticityDamageAmorMieGruneisen::solveStatevar()
{
  Real gmax, gdiff;
  unsigned int iterg;
...
void
FiniteStrainCrystalPlasticityDamageAmorMieGruneisen::postSolveStatevar()
{
  if (_max_substep_iter == 1)//No substepping
  {
    _gss[_qp] = _gss_tmp;
 ...
 */
void
FiniteStrainCrystalPlasticityDamageAmorMieGruneisen::updateGss()
{
...
void
FiniteStrainCrystalPlasticityDamageAmorMieGruneisen::update_energies()
{
...
void
FiniteStrainCrystalPlasticityDamageAmorMieGruneisen::calcResidual( RankTwoTensor &resid )
{
  ...
  if (Je > 1.0) { // only in expansion
    _W0e[_qp] = _G_Gruneisen * _density[_qp] * _specific_heat[_qp] * (temp - _reference_temperature) * V0V
                + Kb * eta * (1.0 - (_G_Gruneisen / 2.0) * (V0V - 1.0)) / std::pow((1.0 - _s_UsUp * eta) , 2.0);    
  } else {
    _W0e[_qp] = 0.0;
  }
...
void
FiniteStrainCrystalPlasticityDamageAmorMieGruneisen::getSlipIncrements()
{
  for (unsigned int i = 0; i < _nss; ++i)
  {
